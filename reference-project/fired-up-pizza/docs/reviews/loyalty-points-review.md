...
**Design spec:** [`docs/designs/loyalty-points-spec.md`](../designs/loyalty-points-spec.md)
**ADR:** [`docs/architecture/loyalty-points-storage.md`](../architecture/loyalty-points-storage.md)
**Reviewer run:** sling 3 (prior two runs flagged issues that were resolved via coder prompt edits — see commits on `packs/lessons/C1/agents/builder/prompt.template.md`)
...
These were flagged in earlier Reviewer runs and resolved by updating `packs/lessons/C1/agents/builder/prompt.template.md`. Kept here for audit.

### (Was Medium-1, now resolved) · `rollback` was not idempotent in sling 1

**Original severity:** Medium
**Resolution:** Coder prompt updated with `"Rollback functions must be idempotent — a second call with the same arguments must be a no-op, not a second rollback."` (commit `a7c3d19` on `packs/lessons/C1/agents/builder/prompt.template.md`). Sling 2 produced a correct idempotent implementation. Verified.
...
**References:** `docs/plans/loyalty-points-system.md`, `docs/architecture/loyalty-points-storage.md`, `docs/designs/loyalty-points-spec.md`

...
7. Create child beads with `bd create --label needs-pm --label source:actual-architect --metadata-field gc.routed_to=l3-project/pm`
8. Mail the pm: `gc mail send pm "Handoff: <topic> complete"`
9. Close the source bead

**Important:** You MUST use `--label needs-pm` AND `--metadata-field gc.routed_to=l3-project/pm` when creating child beads.
